...
def make_generator_model():
  ...
  model.add(layers.Conv2DTranspose(512, kernel_size=4, strides=4, padding='same'))
  model.add(layers.LeakyReLU(alpha=0.1))
  ...
  recurrent_part.add(layers.Dense(512))
...
def make_discriminator_model():
  ...
  model.add(layers.LeakyReLU(alpha=0.1))
  model.add(layers.Conv2D(512, (4, 4), padding='same', strides=2))
  model.add(layers.LeakyReLU(alpha=0.1))
  model.add(layers.Conv2D(512, (4, 4), padding='same', strides=4))
  model.add(layers.LeakyReLU(alpha=0.1))
...
class CustomModel(tf.keras.Model):
  ...
  def inference(self):
    ...
    for _ in range(seq_len):
      ...
      noise = noise + self.recurrent_generator(noise, training=False)
  ...
  def train_step(self, images):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
      ...
      for i in range(seq_len):
        ...
        self.noise = self.noise + self.recurrent_generator(self.noise, training=True)
        gen_outputs.append(self.noise)
        ...
        prev_real_output = prev_real_output + self.recurrent_discriminator(concat_real, training=True)
        ...
        prev_fake_output = prev_fake_output + self.recurrent_discriminator(concat_fake, training=True)
      ...
      #   replace = random.random() <= reset_probability
      #   if replace:
      #     new_noise.append(tf.random.normal((noise_dim,)))
      #   else: 
      #     new_noise.append(self.noise[i,:])
      # self.noise = tf.stack(new_noise)
